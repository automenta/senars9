...
fn test_compound_term_construction_and_formatting() {
    ...
    let inheritance = Term::new_compound(TermType::Inheritance, vec![cat.clone(), mammal.clone()]);
    assert_eq!(inheritance.name, "(cat --> mammal)");

    let similarity = Term::new_compound(TermType::Similarity, vec![dog.clone(), wolf.clone()]);
    assert_eq!(similarity.name, "(dog <-> wolf)");

    let implication = Term::new_compound(TermType::Implication, vec![premise.clone(), conclusion.clone()]);
    assert_eq!(implication.name, "(premise ==> conclusion)");

    let equivalence = Term::new_compound(TermType::Equivalence, vec![cat.clone(), feline.clone()]);
    assert_eq!(equivalence.name, "(cat <=> feline)");

    let conjunction = Term::new_compound(TermType::Conjunction, vec![cat.clone(), furry.clone()]);
    assert_eq!(conjunction.name, "(&, cat, furry)");

    let disjunction = Term::new_compound(TermType::Disjunction, vec![cat.clone(), dog.clone()]);
    assert_eq!(disjunction.name, "(|, cat, dog)");

    let negation = Term::new_compound(TermType::Negation, vec![cat.clone()]);
    assert_eq!(negation.name, "(--, cat)");

    let sequential_conjunction = Term::new_compound(TermType::SequentialConjunction, vec![clean.clone(), dirty_room.clone()]);
    assert_eq!(sequential_conjunction.name, "(&/, clean, dirty_room)");

    let operation = Term::new_compound(TermType::Operation, vec![function.clone(), arguments.clone()]);
    assert_eq!(operation.name, "(function ^ arguments)");

    let product = Term::new_compound(TermType::Product, vec![cat.clone(), dog.clone(), bird.clone()]);
    assert_eq!(product.name, "(cat, dog, bird)");

    // Set and Property Operators
    let instance = Term::new_compound(TermType::Instance, vec![fluffy.clone(), cat.clone()]);
    assert_eq!(instance.name, "(fluffy {-- cat)");

    let property = Term::new_compound(TermType::Property, vec![cat.clone(), furry.clone()]);
    assert_eq!(property.name, "(cat --} furry)");

    let extensional_set = Term::new_compound(TermType::ExtensionalSet, vec![cat.clone(), dog.clone(), bird.clone()]);
    assert_eq!(extensional_set.name, "{cat, dog, bird}");

    let intensional_set = Term::new_compound(TermType::IntensionalSet, vec![furry.clone(), pet.clone()]);
