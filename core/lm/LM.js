...
class LM extends Component {
 ...
 _countTokens = text => {
  ...
  // Delegation methods for neurosymbolic cognitive operations
  async decomposeGoal(goal) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('goal_decomposition', { goal });
    }
    // Fallback - basic response
    return {
      original: `Goal: ${goal} - No specific sub-goals identified`,
      type: 'goal_decomposition',
      error: 'No reasoner available for goal decomposition'
    };
  }

  async generateHypotheses(observations) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('hypothesis_generation', { observations });
    }
    // Fallback - basic response
    return {
      original: `No hypotheses generated for: ${observations}`,
      type: 'hypothesis_generation',
      error: 'No reasoner available for hypothesis generation'
    };
  }

  async groundSymbol(symbol) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('symbol_grounding', { symbol });
    }
    // Fallback - basic response
    return {
      original: `Symbol ${symbol} - no grounding provided`,
      type: 'symbol_grounding',
      error: 'No reasoner available for symbol grounding'
    };
  }

  async explainConclusion(conclusion) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('explanation_generation', { conclusion });
    }
    // Fallback - basic response
    return {
      original: `No explanation available for: ${conclusion}`,
      type: 'explanation_generation',
      error: 'No reasoner available for explanation generation'
    };
  }

  async induceSchema(narrative) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('schema_induction', { narrative });
    }
    // Fallback - basic response
    return {
      original: `No schema induced from: ${narrative}`,
      type: 'schema_induction',
      error: 'No reasoner available for schema induction'
    };
  }

  async estimateConfidence(statement) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('uncertainty_calibration', { statement });
    }
    // Fallback - basic response
    return {
      original: `Confidence estimation not available for: ${statement}`,
      type: 'uncertainty_calibration',
      error: 'No reasoner available for uncertainty calibration'
    };
  }

  async inferTemporalCausal(statement) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('temporal_causal_modeling', { statement });
    }
    // Fallback - basic response
    return {
      original: `No temporal/causal relations identified in: ${statement}`,
      type: 'temporal_causal_modeling',
      error: 'No reasoner available for temporal/causal modeling'
    };
  }

  async recommendReasoningStrategy(problem) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('meta_reasoning', { problem });
    }
    // Fallback - basic response
    return {
      original: `No reasoning strategy recommended for: ${problem}`,
      type: 'meta_reasoning',
      error: 'No reasoner available for meta-reasoning'
    };
  }

  async generateClarifyingQuestions(problem) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('interactive_clarification', { problem });
    }
    // Fallback - basic response
    return {
      original: `No clarifying questions generated for: ${problem}`,
      type: 'interactive_clarification',
      error: 'No reasoner available for interactive clarification'
    };
  }

  async resolveContradiction(contradiction) {
    if (this.reasoner && typeof this.reasoner.performAdvancedReasoning === 'function') {
      return this.reasoner.performAdvancedReasoning('belief_revision', { contradiction });
    }
    // Fallback - basic response
    return {
      original: `No resolution provided for contradiction: ${contradiction}`,
      type: 'belief_revision',
      error: 'No reasoner available for belief revision'
    };
  }

  // Existing delegation methods for reasoner functionality
