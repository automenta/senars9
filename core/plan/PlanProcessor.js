...
class PlanProcessor extends Component {
  constructor(lm = null, htnPlanner = null) {
    ...
    this.config = {
      ...
      enableLMProcessing: DEFAULTS.PLAN_PROCESSOR_ENABLE_LM !== false
    };
  ...
  async _extractWithLM(content, existingGoals, options) {
    ...
    try {
      const prompt = `Extract specific, actionable goals from: ${content}
Return as JSON array with text, confidence, priority fields.`;

      const response = await this.lm.generateText(prompt, { temperature: 0.3, maxTokens: 500 });
