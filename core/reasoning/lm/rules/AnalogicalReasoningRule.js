 ...
 * @description Analogical reasoning rule for the LM Reasoning API
 */
...
export class AnalogicalReasoningRule extends LMRule {
  ...
  generatePrompt(context) {
    ...
    if (!task) {
      throw new Error('No task provided to generate prompt for AnalogicalReasoningRule');
    }
  ...
  generateTasks(processedOutput, context) {
    ...
    if (processedOutput && processedOutput.trim()) {
      ...
      // Create an analogical solution based on the LM's suggestion
      newTasks.push({
        term: `analogical_solution_to_${originalTerm.replace(/[^\w]/g, '_')}`,
        punctuation: '!',  // This would be a goal/solution
        truth: { frequency: 0.7, confidence: 0.6 },
        content: processedOutput
      });
      
      // Create a belief linking the analogy to the original problem
      newTasks.push({
        term: `(analogical_solution_for_${originalTerm.replace(/[^\w]/g, '_')} --> "${processedOutput}").`,
        punctuation: '.',
        truth: { frequency: 0.7, confidence: 0.6 }
      });
    }
