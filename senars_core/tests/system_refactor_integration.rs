...
fn test_system_end_to_end_reasoning() {
    ...
    assert_eq!(system.runtime.memory.short_term_tasks.len(), 3, "Should have 3 initial tasks.");
    ...
    let cat_concept = system.runtime.memory.concept_storage.get(&cat_hash).unwrap();
    ...
    let cat_concept = system.runtime.memory.create_or_get_atom_concept("cat", 0);
    let animal_concept = system.runtime.memory.create_or_get_atom_concept("animal", 0);
    let derived_concept = system.runtime.memory.create_or_get_compound_concept(
    ...
    let derived_task = system.runtime.memory.get_task(&derived_concept.term.hash);
    ...
    let bird_concept = system.runtime.memory.concept_storage.get(&bird_hash).unwrap();
    ...
    assert!(system.runtime.memory.short_term_tasks.len() > 4, "Memory should contain more tasks after multiple cycles.");
}